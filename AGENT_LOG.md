...
Next: keep periodically curl-checking Bonnaroo/chains-app's HEAD for Guillermo's manual OI-2 upload; if it moves, verify the live site (no betting UI/coins chip, Go Throw blank-scoring/scorecard/next-gating per OI-3, no console errors) and mark OI-2 done. OI-2 has now been blocked on the same manual hand-off for 9+ hours across many automated runs — worth a direct nudge to Guillermo next time he's in a live session; same for OI-4's go-ahead+Formspree ID. No other action needed this run.
